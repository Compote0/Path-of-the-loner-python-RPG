...
class Character:
    ...
    def attack_enemy(self, enemy):
        ...
        damage = self.attack
        enemy.hp -= damage
        enemy.hp = max(0, enemy.hp)  # Assure que les HP ne deviennent pas négatifs
        return damage

def main():
    pygame.init()

    # Configuration de l'écran et des titres
    ...
    # Création des textes affichés à l'écran
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key in [pygame.K_ESCAPE, pygame.K_q]:
                    running = False
                elif event.key == pygame.K_a:  # Attaque
